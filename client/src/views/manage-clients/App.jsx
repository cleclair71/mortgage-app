            ...
            <Box w="100%" d="flex" justifyContent="space-between" alignItems="center">
                ...
                <Box d="flex" alignItems="center">
                    <InputGroup size="md" maxW="300px" mr="1rem">
                        ...
                        <Input placeholder="Search clients..." borderRadius="md" />
                    </InputGroup>
                    <IconButton aria-label="Add new client" size="md" icon={<FaPlus />} variant="outline" _hover={{ bgColor: "gray.200" }} />
                </Box>
            </Box>
...
const ClientRow = ({ name, birthdate, contact, renewalDate, loanAmount }) => {
    const id = generate();  // Generate unique ID using shortid
    ...
    return (
        <Tr>
            <Td><Avatar size="sm" bg={randomColor} color="white" name={name}></Avatar></Td>
            <Td>{name}</Td>
            <Td>{birthdate}</Td>
            <Td>{contact}</Td>
            <Td>{renewalDate}</Td>
